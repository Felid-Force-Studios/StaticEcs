    ...
    public abstract partial class World<TWorld> {
        ...
        internal interface IQueryFunctionAdapter<T0, T1, T2>
            ...
            public void InvokeBlock(Entity entity,
                                    ...
                                    int start, int end, Span<T0> comp0, Span<T1> comp1, Span<T2> comp2);
                                    #else
                                    uint start, uint end, T0[] comp0, T1[] comp1, T2[] comp2);
                                    #endif
        }
        ...
        public readonly ref partial struct WorldQuery<TFilter> where TFilter : struct, IQueryFilter {
            ...
            public bool Search<T0, T1, T2>(out Entity entity,
                ...
                if (Prepare<T0, T1, T2>(Filter, clusters, entities, components, strict, out var queryData, out var firstGlobalBlockIdx)) {
                    #if FFS_ECS_DEBUG
                    try
                    #endif
                    {
                    ...
                    #endif
                    {
                        Dispose<T0, T1, T2>(Filter, entities, components, strict, queryData);
                    }
                }
            ...
            internal unsafe void ForBlockInternal<T0, T1, T2, TAdapter>(ref TAdapter adapter,
                ...
                {
                if (Prepare<T0, T1, T2>(Filter, clusters, entities, components, true, out var queryData, out var firstGlobalBlockIdx)) {
                    ref var world = ref Data.Instance;
                    #if FFS_ECS_DEBUG
                    try
                    #endif
                    ...
                    #endif
                    {
                        Dispose<T0, T1, T2>(Filter, entities, components, true, queryData);
                    }
                }
            }
            ...
            #if !FFS_ECS_DISABLE_CHANGED_TRACKING
            [MethodImpl(AggressiveInlining)]
            internal void ForInternal<T0, T1, T2, TFunction, TTracker>(ref TFunction function,
                TTracker tracker,
                EntityStatusType entities = EntityStatusType.Enabled,
                ComponentStatus components = ComponentStatus.Enabled,
                QueryMode queryMode = QueryMode.Strict,
            ...
            {
                if (tracker.IsActive) {
                    ForInternal<T0, T1, T2, TFunction, TTracker>(ref function, tracker, entities, components, queryMode, clusters);
                } else {
                    ForInternal<T0, T1, T2, TFunction>(ref function, entities, components, queryMode, clusters);
                }
            }
            #endif
            ...
            [SuppressMessage("ReSharper", "PossibleNullReferenceException")]
            [MethodImpl(AggressiveInlining)]
            internal bool Prepare<T0, T1, T2>(TFilter filter, ReadOnlySpan<ushort> clusters, EntityStatusType entities, ComponentStatus components,
            ...
            [MethodImpl(AggressiveInlining)]
            internal void Dispose<T0, T1, T2>(TFilter filter, EntityStatusType entities, ComponentStatus components, bool strict, QueryData queryData)
                where T0 : struct, IComponent
                where T1 : struct, IComponent
                where T2 : struct, IComponent {
                ref var world = ref Data.Instance;

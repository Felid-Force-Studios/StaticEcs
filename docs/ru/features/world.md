...
- Разные кластеры могут определять разные типы сущностей, например кластер юнитов, кластер игрового окружения, кластер предметов, кластер эффектов  
  - Это позволяет уменьшить потребление и фрагментацию памяти, ускорить итерацию, и помогать в сериализации мира и игровой логике   
  - Например при большом игровой карте, которая подгружается и выгружается по мере движения игрока, разные кластеры сильно экономят память  
- Другой пример это использование кластеров для разных игровых уровней, и загружать\выгружать кластеры при смене уровня  
